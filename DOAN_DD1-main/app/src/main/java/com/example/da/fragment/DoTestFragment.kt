...
class DoTestFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (questions.isEmpty()) {
            Toast.makeText(context, "Không có câu hỏi cho bài test này.", Toast.LENGTH_SHORT).show()
            parentFragmentManager.popBackStack()
            return
        }
    ...
    private fun finishTest() {
        ...
        for (question in questions) {
            val correctAnswerIds = dbHelper.getAnswersForQuestion(question.id).filter { it.isCorrect }.map { it.id }.toSet()
